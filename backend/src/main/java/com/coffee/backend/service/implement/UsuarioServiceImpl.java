   ...
   final RolRepository rolRepository;
    ...
    public void registrarUsuario(RegistrarUsuarioRequestDTO requestDTO) {
     ...
     public List<UsuarioResponseDTO> obtenerUsuarios(){
      return usuarioRepository.findByRoles_NombreRol("MESERO").stream()
              .map(list ->
              {
                  return new UsuarioResponseDTO(list.getNombreUsuario(),
                          list.getApellidoUsuario(),list.getCorreoUsuario(),list.getTelefonoUsuario());
              }).toList();
     }
